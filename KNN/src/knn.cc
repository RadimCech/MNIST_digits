...
void knn::find_knearest(data *query_point)
{
 ...
 for(int i = 0; i < k; i++)
 {
    if(i==0)
    {
        for(int j = 0; j , j<training_data->size(); j++)
        {
...
double knn::calculate_distance(data* query_point, data* input)
{
    ...
    //put manhatten implementation here
    #endif
    return distance;
}
double knn::validate_performance()
{
    ...
    current_performance = ((double)count*100.)/((double)validation_data->size());
    printf("Validation performance = %d %.3f %%\n", k, current_performance);
